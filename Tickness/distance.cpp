...
void Distance::GetTickness(int first, int last){
    ...
    if (extractValues.is_open()){
        for(int i =first; i<last;i++){
            ...
            //            distance[4] = calcDistance(val, first, last, 4);
            //            distance[5] = calcDistance(val, first, last, 5);
            //            distance[6] = calcDistance(val, first, last, 6);
            //            distance[7] = calcDistance(val, first, last, 7);
...
double Distance::calcDistance(ImageType2D::Pointer image, int x, int y, int pos){
    ...
    cout<<"Value = "<<p0.EuclideanDistanceTo(p1)<<endl;
